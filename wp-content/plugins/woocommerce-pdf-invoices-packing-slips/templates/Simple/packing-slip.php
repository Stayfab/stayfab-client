...
<h2>DIT TILBUD ER BLEVET KØBT</h2>
Vi håber, at din kunde bliver glad for sin behandling.<br><br>
   ...
   <?php $moms_val = ($order->get_total()*25)/100; ?>
        ...
        <!-- Shipping -->
            ...
            <tr class="shipping after-products">
                <td colspan="4"></td>
                <td><?php _e( 'Transfer fee (15,000%)'); ?></td>
                <td class="align-right"><?php echo number_format(($order->get_total()*15)/100).'DKK' ?></td>
            </tr>
     
        <!-- Subtotal -->
 <?php
//$vendor_name = $user_info->user_login;  get_userdata($author_id);
$vendor_name  = get_user_meta($author_id,'vendor_name',true);
...
$ord_tot = $order->get_total() - $order->get_total_tax();
 ...
 ?>      
            <tr class="subtotal after-products">
                <td colspan="4"></td>
                <td><?php _e( 'Til udbetaling'); ?></td>
                <td class="align-right"><?php  echo $commission_val.'DKK'.$percentage_text ; ?></td>
            </tr>
