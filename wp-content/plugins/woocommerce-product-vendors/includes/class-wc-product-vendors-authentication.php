...
class WC_Product_Vendors_Authentication {
	...
	public function set_cookie( $user_id, $vendor_id ) {
		...
		return setcookie( 'woocommerce_pv_vendor_id_' . COOKIEHASH, absint( $vendor_id ), 0, SITECOOKIEPATH, COOKIE_DOMAIN );
	}
	...
	public function expire_cookie() {
		return setcookie( 'woocommerce_pv_vendor_id_' . COOKIEHASH, ' ', current_time( 'timestamp' ) - YEAR_IN_SECONDS, SITECOOKIEPATH, COOKIE_DOMAIN );
	}
	...
	public function login_authentication( $user, $username, $password ) {
		...
		if ( ! is_wp_error( $user ) && WC_Product_Vendors_Utils::is_vendor( $user->ID ) ) {
			...
			if ( WC_Product_Vendors_Utils::is_pending_vendor( $user->ID ) ) {
				$user = new WP_Error( 'error', __( 'Din ansøgning venter på godkendelse.  Du får en mail fra Stayfab når din konto bliver godkendt.', 'woocommerce-product-vendors' ) );
