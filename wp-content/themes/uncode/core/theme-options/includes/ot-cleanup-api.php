  ...
  class OT_Cleanup {
    ...
    public function cleanup_notice() {
      ...
      if ( get_current_screen()->id != 'appearance_page_ot-cleanup' )
        echo '<div class="update-nag">' . sprintf( wp_kses(__( 'OptionTree has outdated data that should be removed. Please go to %s for more information.', 'option-tree' ), array( 'a' => array( 'href' => array() ) ) ), sprintf( '<a href="%s">%s</a>', admin_url( 'themes.php?page=ot-cleanup' ), apply_filters( 'ot_cleanup_menu_title', esc_html__( 'OptionTree Cleanup', 'option-tree' ) ) ) ) . '</div>';
    ...
    public function options_page() {
      ...
      echo '<div class="wrap">';
        ...
        if ( $ot_maybe_cleanup_posts ) {
          ...
          echo '<p>' . sprintf( wp_kses(__( 'There are currently %s OptionTree media posts in your database. At some point in the past, a version of OptionTree added multiple %s media post objects cluttering up your %s table. There is no associated risk or harm that these posts have caused other than to add size to your overall database. Thankfully, there is a way to remove all these orphaned media posts and get your database cleaned up.', 'option-tree' ), array( 'code' => array(), 'tt' => array() ) ), '<code>' . number_format( count( $posts ) ) . '</code>', '<tt>option-tree</tt>', '<tt>' . $wpdb->posts . '</tt>' ) . '</p>';

          echo '<p>' . sprintf( wp_kses(__( 'By clicking the button below, OptionTree will delete %s records and consolidate them into one single OptionTree media post for uploading attachments to. Additionally, the attachments will have their parent ID updated to the correct media post.', 'option-tree' ), array( 'code' => array() ) ), '<code>' . number_format( count( $posts ) - 1 ) . '</code>' ) . '</p>';
          ...
          if ($safe_mode) echo '<p>' . sprintf( wp_kses(__( '%s Your server is running in safe mode. Which means this page will automatically reload after deleting %s posts, you can filter this number using %s if your server is having trouble processing that many at one time.', 'option-tree' ), array( 'strong' => array(), 'tt' => array() ) ), '<strong>Note</strong>:', apply_filters( 'ot_consolidate_posts_reload', 500 ), '<tt>ot_consolidate_posts_reload</tt>' ) . '</p>';
        ...
        if ( $ot_maybe_cleanup_table ) {
          ...
          echo '<p>' . sprintf( wp_kses(__( 'If you have upgraded from an old 1.x version of OptionTree at some point, you have an extra %s table in your database that can be removed. It\'s not hurting anything, but does not need to be there. If you want to remove it. Click the button below.', 'option-tree' ), array( 'tt' => array() ) ), '<tt>' . $table_name . '</tt>' ) . '</p>';
          ...
          if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'drop-table' ) ) {
            ...
            echo '<p>' . sprintf( wp_kses(__( 'Deleting the outdated and unused %s table...', 'option-tree' ), array( 'tt' => array() ) ), '<tt>' . $table_name . '</tt>' ) . '</p>';

            $wpdb->query( "DROP TABLE IF EXISTS $table_name" );

            if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $table_name ) ) != $table_name ) {

              echo '<p>' . sprintf( wp_kses(__( 'The %s table has been successfully deleted. The page will now reload...', 'option-tree' ), array( 'tt' => array() ) ), '<tt>' . $table_name . '</tt>' ) . '</p>';
              ...
              echo '<p>' . sprintf( wk_ses(__( 'Something went wrong. The %s table was not deleted.', 'option-tree' ), array( 'tt' => array() ) ), '<tt>' . $table_name . '</tt>' ) . '</p>';
