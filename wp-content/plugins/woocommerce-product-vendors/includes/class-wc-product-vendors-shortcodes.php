...
class WC_Product_Vendors_Shortcodes {
	...
	public function render_registration_shortcode( $atts ) {
		...
		if ( WC_Product_Vendors_Utils::is_vendor() ) {
			return sprintf( '<p class="woocommerce-message">' . __( 'Du er allerede logget ind som en vendor. Se din %1$sVendor Dashboard%2$s', 'woocommerce-product-vendors' ) . '</p>', '<a href="' . esc_url( home_url().'/vendor-dashboard' ) . '">', '</a>' );
