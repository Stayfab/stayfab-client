    ...
    echo '<form method="post" id="option-tree-settings-form">';
      ...
      echo '<div class="format-setting type-textblock has-desc">';
        ...
        echo '<div class="description">';
          ...
          echo '<p class="warning">' . sprintf( wp_kses(__( 'Go to the %s page if you want to save data, this page is for adding settings.', 'option-tree' ), array( 'a' => array( 'href' => array() ), 'code' => array() ) ), '<a href="' . get_admin_url( $blog_id, apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) . '?page=' . apply_filters( 'ot_theme_options_menu_slug', 'ot-theme-options' ) ) . '"><code>Appearance->Theme Options</code></a>' ) . '</p>';
          echo '<p class="warning">' . sprintf( wp_kses(__( 'If you\'re unsure or not completely positive that you should be editing these settings, you should read the %s first.', 'option-tree' ), array( 'a' => array( 'href' => array() ), 'code' => array() ) ), '<a href="' . get_admin_url( $blog_id, 'admin.php?page=ot-documentation' ) . '"><code>Options Utils->Documentation</code></a>' ) . '</p>';
    ...
    echo '<form method="post" id="export-settings-file-form">';
      ...
      echo '<div class="format-setting type-textarea simple has-desc">';
        ...
        echo '<div class="description">';
          ...
          echo '<p>' . sprintf( wp_kses(__( 'Export your Settings into a fully functional %s file. If you want to add your own custom %s text domain to the file, enter it into the text field before exporting. For more information on how to use this file read the documentation on %s. Remember, you should always check the file for errors before including it in your theme.', 'option-tree' ), array( 'a' => array( 'href' => array(), 'target' => array() ), 'code' => array() ) ), '<code>theme-options.php</code>', '<a href="http://codex.wordpress.org/I18n_for_WordPress_Developers" target="_blank">I18n</a>', '<a href="' . get_admin_url( $blog_id, 'admin.php?page=ot-documentation#section_theme_mode' ) . '">' . esc_html__( 'Theme Mode', 'option-tree' ) . '</a>' ) . '</p>';
  ...
  function ot_type_export_settings() {
    ...
    echo '<div class="format-setting type-textarea simple has-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . wp_kses(__( 'Export your Settings by highlighting this text and doing a copy/paste into a blank .txt file. Then save the file for importing into another install of WordPress later. Alternatively, you could just paste it into the <code>Options Utils->Settings->Import</code> <strong>Settings</strong> textarea on another web site.', 'option-tree' ), array( 'strong' => array( ), 'code' => array() ) ) . '</p>';
  ...
  function ot_type_export_data() {
    ...
    echo '<div class="format-setting type-textarea simple has-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . wp_kses(__( 'Export your Theme Options data by highlighting this text and doing a copy/paste into a blank .txt file. Then save the file for importing into another install of WordPress later. Alternatively, you could just paste it into the <code>Options Utils->Settings->Import</code> <strong>Theme Options</strong> textarea on another web site.', 'option-tree' ), array( 'strong' => array( ), 'code' => array() ) ) . '</p>';
    ...
    echo '<form method="post" id="option-tree-settings-form">';
      ...
      echo '<div class="format-setting type-textarea has-desc">';
        ...
        echo '<div class="description">';
          ...
          if ( OT_SHOW_DOCS ) echo '<p>' . wp_kses(__( 'Visit <code>Options Utils->Documentation->Layouts Overview</code> to see a more in-depth description of what layouts are and how to use them.', 'option-tree' ), array( 'code' => array() ) ) . '</p>';
