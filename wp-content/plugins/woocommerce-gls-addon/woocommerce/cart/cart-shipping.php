...
<tr class="shipping">
	<th>
		<small style="font-size:70%;">Fri fragt ved køb over 250 DKK</small>
		<br/>
		<?php
		if ( $show_package_details ) {
			printf( __( 'Shipping #%d', 'woocommerce' ), $index + 1 );
		} else {
			_e( 'Forsendelse', 'woocommerce' );
		}
	?></th>
	<td>
				...
				<ul id="shipping_method">
					<?php foreach ( $available_methods as $method ) : ?>
						<li class="<?php echo $method->id ; ?>">
							<input type="radio" name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>_<?php echo sanitize_title( $method->id ); ?>" value="<?php echo esc_attr( $method->id ); ?>" <?php checked( $method->id, $chosen_method ); ?> class="shipping_method" onchange="return changeMethod(this);" />
							<label for="shipping_method_<?php echo $index; ?>_<?php echo sanitize_title( $method->id ); ?>"><?php echo wp_kses_post( str_replace(":", "" , wc_cart_totals_shipping_method_label( $method ) )); ?></label>
						</li>
					<?php endforeach; ?>
				</ul>
			<?php endif; ?>
